...
public class GLView extends SurfaceView implements SurfaceHolder.Callback {
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			if (eventListener != null) {
				return eventListener.onTouchDown(event.getX(), event.getY());
			}
		} else if (event.getAction() == MotionEvent.ACTION_UP) {
			if (eventListener != null) {
				return eventListener.onTouchUp(event.getX(), event.getY());
			}
		} else if (event.getAction() == MotionEvent.ACTION_MOVE) {
			if (eventListener != null) {
				return eventListener.onTouchMove(event.getX(), event.getY());
			}
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (eventListener != null && keyCode != KeyEvent.KEYCODE_BACK) {
		...
		private void guardedRun() throws InterruptedException {
			...
			while (!mDone) {
				...
				if (tellRendererSurfaceChanged) {
					if (w != 0 && h != 0) {// otherwise GLU.gluPerspective make
											// error
